...
fn test_error_exit_codes() {
    ...
    assert_eq!(
        RatpmError::PackageNotFound("vim".to_string()).exit_code(),
        1
    );
    assert_eq!(
        RatpmError::DependencyConflict("conflict".to_string()).exit_code(),
        2
    );
    assert_eq!(
        RatpmError::TransactionCheckFailed("check".to_string()).exit_code(),
        3
    );
    assert_eq!(
        RatpmError::TransactionFailed("failed".to_string()).exit_code(),
        4
    );
    assert_eq!(
        RatpmError::NetworkError("network".to_string()).exit_code(),
        5
    );
    assert_eq!(
        RatpmError::RepoUnavailable("repo".to_string()).exit_code(),
        6
    );
...
fn test_error_messages() {
    ...
    assert_eq!(
        err.to_string(),
        "Permission denied: operation requires root privileges"
    );
    ...
    assert_eq!(
        err.to_string(),
        "Package manager lock is held by another process (PID: 1234)"
    );
}
