...
pub fn resolve_install(
    ...
    for pkg_name in packages {
        let pkg_info = repos.get_package_info(pkg_name).context(format!(
...
pub fn resolve_remove(rpm_db: &RpmDatabase, packages: &[String]) -> Result<Transaction> {
    ...
    for pkg_name in packages {
        ...
        if !dependents.is_empty() {
            ...
            return Err(RatpmError::DependencyConflict(format!(
...
pub fn resolve_upgrade(repos: &RepositoryManager, rpm_db: &RpmDatabase) -> Result<Transaction> {
...
fn find_dependents(_package: &str, _rpm_db: &RpmDatabase) -> Result<Vec<String>> {
