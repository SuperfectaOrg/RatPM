use crate::backend::fedora::types::{DiagnosticIssue, Package, PackageInfo, RepositoryMetadata};
...
impl RepositoryManager {
    ...
    fn parse_repo_file(&self, path: &PathBuf) -> Result<Vec<RepositoryMetadata>> {
        let content = fs::read_to_string(path).context("Failed to read repository file")?;
        ...
        for line in content.lines() {
            ...
            if let Some(ref mut repo_data) = current_repo {
                if let Some((key, value)) = line.split_once('=') {
                    repo_data.insert(key.trim().to_string(), value.trim().to_string());
                }
            }
        }
    ...
    fn build_repository(
        ...
        let gpgcheck = data
            .get("gpgcheck")
            .and_then(|v| v.parse::<i32>().ok())
            .unwrap_or(if self.gpgcheck { 1 } else { 0 })
            == 1;

        let gpgkey: Vec<String> = data
            .get("gpgkey")
    ...
    pub fn search(&self, query: &str) -> Result<Vec<Package>> {
        ...
        results.dedup_by(|a, b| a.name == b.name && a.version == b.version && a.arch == b.arch);
    ...
    fn search_cache(&self, _cache_file: &PathBuf, query: &str) -> Result<Vec<Package>> {
        ...
        let mock_packages = vec![
            ...
            (
                ...
                "9.0.2190",
                "x86_64",
                "The improved version of the vi editor",
            ),
