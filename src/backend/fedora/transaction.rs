...
pub fn execute_transaction(
    ...
    tracing::info!(
        "Executing transaction with {} total operations",
        transaction.total_packages()
    );
...
fn download_packages(
    ...
    for spec in packages_to_download {
        ...
        fs::write(&rpm_path, b"MOCK_RPM_DATA").context("Failed to write package file")?;
    }

    Ok(())
}

fn verify_package_signatures(transaction: &Transaction, packages_dir: &Path) -> Result<()> {
    ...
    tracing::info!(
        "Verifying signatures for {} packages",
    ...
    for spec in packages_to_verify {
        ...
        if !rpm_path.exists() {
            return Err(RatpmError::BackendError(format!(
